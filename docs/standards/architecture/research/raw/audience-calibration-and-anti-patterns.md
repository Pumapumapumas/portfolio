...
What this paper concludes, in one line: **the three audiences do not conflict on *substance*, they conflict on *depth, register, and ordering* — and all three of those are resolvable by layering rather than by splitting, with exactly one genuine exception (a sales surface), which the evidence says to defer.** That conclusion is **[derived]** from the sources in §4–§7 and is the paper's own inference, not any source's finding.
...
- *"Use restraint with any promotional content"* [S9] — note that this is a *client-facing* guideline that constrains client-facing copy.
...
**A3 — Promotional register / boasting.** *Damage: all three, measurably.* The control condition in [S2] — *"exaggeration, subjective claims, and boasting, rather than just simple facts"* — scored worst on virtually all usability measures, and the objective rewrite of the same content scored 27% higher [S2] **[definitive** for that study]. Stanford guideline 9 independently advises restraint with promotional content and to *"be clear, direct, and sincere"* [S9]. NN/g's plain-language research names the belief that drives it: *"The misconceived notion that long sentences and big words make you sound smarter (or more professional) results in great sacrifices to readability and credibility."* [S7]
...
**Present in this repo:** four tiles carry 4–6 tags each and, currently, zero reachable evidence.
...
**A6 — Stale content.** *Damage: all three; clients most, because staleness reads as "no longer trading".* Stanford guideline 8: *"Update your site's content often (at least show it's been reviewed recently)."* [S9] **[definitive]**. The subject's site is especially exposed: a project set dated "2025–26" with no status markers ages into ambiguity by default.
...
**A9 — A sales surface that changes the site's genre.** *Damage: peer standing; the direction of damage is asymmetric.* This is C3. Evidence is indirect but convergent: promotional content is what [S2] measured worst and what [S9 g9] tells you to restrain — and both of those are *general-audience* findings, so the peer-specific magnitude is unmeasured. **[derived**, and explicitly flagged in §11 (G6) as the paper's weakest inference.]
...
| **P4** | **Rewrite hero + About into objective register.** Delete *enterprise-grade* and every self-rating adjective; replace with checkable nouns. | All three | [S2], [S7], [S9 g9] | ~90 words is the entire budget that gets read [S1]. |
...
| **P11** | **Do NOT build a services/rates/testimonials surface in this pass.** | — | [S9 g9], [S2], [S7] | Deliberately deferred to topic 9, which has the diagnosis this paper supplies. See §11 G6: the peer-standing cost is **unmeasured**, so this is a low-confidence recommendation and should be re-decided with topic 9's evidence, not treated as settled. |
...
*Method:* fetched the 2024 survey landing page and enumerated its five result sections, grepping each for "portfolio":
```
...
*Method:* no study found that measures how technical evaluators react to commercial framing on an engineer's personal site; the inference in A9/C3 is assembled from general-audience findings [S2][S9 g9]. **Result: §9 P11 is the paper's lowest-confidence recommendation.** It is topic 9's question and this paper deliberately does not settle it.
...
**Quotation discipline used in this paper.** Every span presented as a quotation was returned as exact characters by a direct `curl` fetch of the source URL, converted from HTML with a local tag-stripping parser; HTML source line-wrapping is normalised to single spaces and nothing else is altered. **PDF sources ([S10], [S11], [S12], [S13]) mangle punctuation and word spacing in their text layers**, so the default for them is **paraphrase, explicitly labelled as paraphrase**. Three spans from PDFs are nonetheless presented as quotations, and only because each was re-verified by literal substring match against the extracted characters after the paraphrase policy was set: the two [S10] spans in §2.4, and the tweet quoted inside [S12] in §4.1. Every other PDF-derived statement in this paper is a paraphrase. Where a source's apostrophes are typographic (U+2019), quotations reproduce them as such; this was byte-checked for the [S7] and [S16] spans. No claim in this paper is sourced to a search-engine result summary; searches were used only to locate sources, which were then fetched.
...
| S9 | Fogg, B.J. (2002). *Stanford Guidelines for Web Credibility.* Stanford Persuasive Technology Lab. https://credibility.stanford.edu/guidelines/index.html | First-party research summary (3 years, 4,500+ people) | The 10 guidelines; errors, contact, expertise, recency, promotional restraint |
...
| S20 | Stack Overflow Developer Survey 2024 — results and methodology. https://survey.stackoverflow.co/2024/ and https://survey.stackoverflow.co/2024/methodology | First-party survey (used as a **negative** finding) | G3: zero "portfolio" matches across six enumerated documents; self-selected sample |
