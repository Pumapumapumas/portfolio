...
§8's whole case rests on [S12]'s assessment-vs-conventional distinction, which depends on artifacts being **costly to produce**. That study is from 2013. In 2026, plausible-looking code, READMEs, architecture prose and even case studies are cheap to generate — which, if peers have internalised it, moves "a well-written repo" from assessment signal toward conventional signal, and shifts the discriminating evidence toward things that are still expensive: a *running* system, third-party corroboration, and specific failure narratives. **I found no study measuring this shift** (§11 G7), so the concern is stated as **[derived, unmeasured]** — but it is the most likely way this paper goes stale, and it is why the revalidation interval is the Medium band's minimum rather than its maximum.
